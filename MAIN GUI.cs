...
public class MAINGUI : BaseUnityPlugin
{
    ...
    void W(int id)
    {
        ...
        if (platss)
        {
            ...
            {
                PlatsLeft.transform.position = GorillaLocomotion.GTPlayer.Instance.leftControllerTransform.position;
                PlatsLeft.transform.rotation = GorillaLocomotion.GTPlayer.Instance.leftControllerTransform.rotation;
                PlatLSpawn = true;
            }
            if (ControllerInputPoller.instance.rightControllerGripFloat > 0.1f && PlatsRight != null && !PlatRSpawn)
            {
                PlatsRight.transform.position = GorillaLocomotion.GTPlayer.Instance.rightControllerTransform.position;
                PlatsRight.transform.rotation = GorillaLocomotion.GTPlayer.Instance.rightControllerTransform.rotation;
                PlatRSpawn = true;
            }
